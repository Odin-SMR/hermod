...
Copy level1b files to current directory.
"""
...
def main():
    ...
    parser = OptionParser(
            ...
            version="%%prog, Hermod release %s" % (config.get('DEFAULT',
                                                              'version')), 
            description="Copy level1b files."
            )
    ...
    parser.add_option('-s', '--start-time', 
            action='callback', callback=parsetime, dest='datestart', 
            nargs=2, type='string', metavar='YYYYMMDD HH:MM', 
    ...
    try:
        status = cursor.execute('''
			select filename,logname from level1 
   			where 1 
			and orbit>=%s 
            and start_utc>=%s
            and orbit<=%s
            ...
            ''', (options.orbitstart, options.datestart, options.orbitend, 
            options.dateend, options.fm, options.cal))
    ...
    for i in cursor:
            try:
                if not i['filename'] is None: 
                    src = join(config.get('GEM', 'LEVEL1B_DIR'), 
                    	splitext(i['filename'])[0])
                    dst = join(getcwd(), basename(src))
                    #print "cp",src,dst
                    if exists(src):
                        copyfile(src, dst)
